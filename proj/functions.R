...
run_interactive_glm_model <- function(
  ...
  for(k in lags) {
    ...
    if (is.null(model_result)) {
      ...
      model_result_collector[[length(model_result_collector) + 1]] <- c(
        NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, model_metadata
      )
    } else {
      model_metadata <- list(c(
        model = model_result["call"],
...
run_interactive_gaseousconf_glm_model <- function(
  city,
  data,
  outcome,
  exposure,
...
run_metafor_noninteractive <- function(
  data,
  exposure,
  outcome,
  lags
...
run_metafor_bisection <- function(
  data,
  exposure,
  outcome,
  quantile_type,
...
create_noninteractive_plot <- function(
  exposure,
  outcome,
  lags,
  data,
  ...
  # subset data
  data <- data[data["exposure"] == exposure & data["outcome"] == outcome & data$lag %in% lags,]
  data$facet_group <- ifelse(data$lag %in% c("0", "1", "2", "3", "4", "5"), "Lags", "Moving Averages")
  data$lag <- recode(
    data$lag,
     "ma1" = "0-1",
     "ma2" = "0-2",
  ...
  data$lag <- factor(data$lag, levels = c(
  ...
  x <- ggplot(data, aes(x = lag, y = rr)) +
    geom_errorbar(aes(ymin = cil, ymax = ciu), color = "black", width = 0.4) +
    geom_hline(yintercept = 1.0, linetype = "dotted", color = "black") +
    geom_point(size = 2.5, color=point_color, shape=point_shape) +
    labs(x = NULL,
    ...
    scale_y_continuous(limits = c(lower_bound, upper_bound), labels = label_number(accuracy = 0.01)) +
    facet_grid(~ facet_group, scales = "free_x", space = "free")

  return(x)
}

create_interactive_plot <- function(
  exposure,
  outcome,
  lags,
  data,
  ...
  for (i in seq(length(data))) {
    tmp <- data[[i]]
    tmp <- tmp[tmp["exposure"] == exposure & tmp["outcome"] == outcome & tmp$lag %in% lags,]
  ...
  x_axis_levels <- paste0(
    rep(cutoffs_vec, times = 6), 
    "Q", rep(rep(1:2, each = 4), times = 3), 
    "L", rep(0:2, each = 8)
  )
  ...
  x <- ggplot(data, aes(x = x_axis, y = rr, shape = as.factor(cutoffs), color = as.factor(cutoffs))) +
    geom_errorbar(aes(ymin = cil, ymax = ciu), color = "black", width = 0.75) +
    geom_hline(yintercept = 1.0, linetype = "dotted", color = "black") +
    geom_point(size = 2.5) +
    labs(x = "Lag 0      Lag 0      Lag 1      Lag 1      Lag 2      Lag 2",
         y = "RR per IQR Increase",
         title = " ",
         shape = legend_label,
         color = legend_label) +
    ...
    facet_wrap(~ group, scales = "free_x", nrow = 1, labeller = group_labeller) +
